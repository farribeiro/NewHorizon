...
local function update_body_textures(player)
    ...
    -- Atualiza textura do corpo visível
    body:set_properties({
        textures = textures
    })
end
...
local function update_belt_items(player)
    ...
    -- Se não houver cinto equipado, retornar sem criar entidades
    if belt_stack:is_empty() then
        return
    end
    
    -- Opcional: verificar se o item é realmente um cinto
    local belt_item = belt_stack:get_name()
    if not belt_item:match("belt") then -- Ajuste conforme a nomenclatura dos seus cintos
        return
    end
...
local function update_offhand_item(player)
    ...
    if item_def then
        ...
        if item_def.offhand_bone_position then
            if item_def.offhand_bone_position.pos then
                final_pos = item_def.offhand_bone_position.pos
            end
            if item_def.offhand_bone_position.rot then
                final_rot = item_def.offhand_bone_position.rot
            end
...
local function get_armor_formspec(player_name)
    ...
    if has_backpack then
        formspec = formspec ..
            "list[current_player;main;0.5,5.7;8,2;8]" ..
        ...
        if not stack:is_empty() then
            ...
            if item_def then
                ...
                if item_def.armor_bone_position then
                    if item_def.armor_bone_position.pos then
                        final_pos = item_def.armor_bone_position.pos
                    end
                    if item_def.armor_bone_position.rot then
                        final_rot = item_def.armor_bone_position.rot
                    end
                end
                
                -- Suporte para armor_visual_size
                if item_def.armor_visual_size then
                    final_size = item_def.armor_visual_size
                end
            end
...
local function apply_custom_model(player)
    ...
    -- Torna o nametag invisível (opcional)
...
local function trigger_punch(player)
    ...
    -- evita spam
    if punch_timers[name] then return end

    if has_item then
        set_player_animation(player, "holding_punch")
    else
        set_player_animation(player, "punch")
    end
...
set_player_animation = function(player, anim)
    ...
    if anim_data then
        -- Aplica animação no player invisível
        player:set_animation(anim_data[1], anim_data[2], anim_data[3], anim_data[4])
        
        -- Aplica a MESMA animação no corpo visível
        local body = body_entities[player_name]
        if body and body:get_luaentity() then
            body:set_animation(anim_data[1], anim_data[2], anim_data[3], anim_data[4])
        end
    end
end
...
core.register_on_placenode(function(pos, newnode, placer)
    ...
    -- animação única de place
    trigger_punch(placer)


    -- inicia loop se ainda não estiver ativo
    if not punch_loop_timers[name] then
        trigger_punch_loop(placer)
    end
end)
...
core.register_globalstep(function(dtime)
	...
	if last_sneak[player_name] ~= ctrl.sneak then
        ...
        if ctrl.LMB then
            ...
            if last_lmb[name] then
                stop_punch_loop(player)
            end
        end
        ...
        if belt_changed then
            ...
            if not armor_changed then  -- Só chama se armor_changed não chamou antes
        ...
        if last_backpack_state[player_name] == nil then
            last_backpack_state[player_name] = false
        end
        ...
        if punch_timers[player_name] then
            goto continue
        end
