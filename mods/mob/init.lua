...
local function get_self_id(self)
    return tostring(self.object)
end

local function assign_slot(pname, self_id)
    _boss_slots[pname] = _boss_slots[pname] or {}
    if _boss_slots[pname][self_id] then
        return _boss_slots[pname][self_id]
    end
    -- Acha o menor slot livre
    local used = {}
    for _, slot in pairs(_boss_slots[pname]) do
        used[slot] = true
    end
    local slot = 0
...
local function free_slot(pname, self_id)
    if _boss_slots[pname] then
        _boss_slots[pname][self_id] = nil
    end
end
...
local function update_boss_hud(self, boss_name)
    ...
    for _, player in ipairs(minetest.get_connected_players()) do
        ...
        if dist > 40 then
            ...
            if not self._hud_ids[pname] then
                -- Pega slot e calcula offset Y
                ...
                player:hud_change(self._hud_ids[pname].text, "text",
                    display_name .. "  " .. hp .. " / " .. hp_max)
            end
        end
    end
end
...
mobs:register_mob("nh_mob:hedgehog", {
    ...
    on_rightclick = function(self, clicker)
        if clicker:is_player() then
            core.chat_send_player(clicker:get_player_name(), S("The hedgehog is friendly, but I need to be careful if I attack it..."))
        end
    end,
...
do_custom = function(self, dtime)
    ...
    for _, obj in ipairs(objetos) do
        if obj ~= self.object then
            ...
            -- Dano em mobs próximos
            if ent and ent._cmi_is_mob and ent.name ~= "nh_mob:urchin" then
                obj:punch(obj, 1.0, {
                    full_punch_interval = 1.0,
                    damage_groups = {fleshy = 5}
                }, nil)
    ...
    for x = -r, r do
        for y = -r, r do
            for z = -r, r do
                ...
                -- Só a casca: entre r-1 e r (espessura de 1 node)
                if dist >= r - 1 and dist <= r then
                    ...
                    if node.name == "air" then
                        minetest.set_node(pos, {name = "nh_nodes:barrier"})
...
function remove_dome(placed_list)
    ...
    for _, pos in ipairs(placed_list) do
        local node = minetest.get_node(pos)
        if node.name == "nh_nodes:barrier" then
...
mobs:register_mob("nh_mob:sentinel", {
    ...
    if ndef and ndef.groups and ndef.groups.liquid then
        ...
        if v and v.y < 0.5 then
            self.object:set_velocity({x=v.x, y=0.8, z=v.z})
        end
    end
...
local objetos = get_objects_in_box(centro, 2, 4, 2)
    ...
    for _, obj in ipairs(objetos) do
        if obj ~= self.object then
            ...
            -- Dano em mobs próximos
            if ent and ent._cmi_is_mob and ent.name ~= "nh_mob:sentinel" then
                obj:punch(obj, 1.0, {
                    full_punch_interval = 1.0,
                    damage_groups = {fleshy = 5}
                }, nil)
...
minetest.register_on_dieplayer(function(player, reason)
    local pname = player:get_player_name()
    for _, obj in ipairs(minetest.get_objects_inside_radius(player:get_pos(), 150)) do
        local ent = obj:get_luaentity()
        ...
        if ent and ent.name == "nh_mob:sentinel" then
            ...
            if pos then
                minetest.set_node(pos, {name = "nh_nodes:sentinelstatue"})
            end
        ...
        if ent and ent.name == "nh_mob:giantcrab" then
            ...
            local pos = obj:get_pos()
            if pos then
                minetest.set_node(pos, {name = "nh_nodes:giantcrabstatue"})
            end
            mobs:remove(ent, true)
        end
    end
...
mobs:register_mob("nh_mob:bubble", {
    ...
    if ndef and ndef.groups and ndef.groups.liquid then
        ...
        for name, _ in pairs(self._player_timers) do
            if not players_above[name] then
                self._player_timers[name] = nil
            end
        end
    end,
})
...
mobs:register_mob("nh_mob:karibo", {
    ...
    on_rightclick = function(self, clicker)
        if clicker:is_player() then
            core.chat_send_player(clicker:get_player_name(), S("The Karibo looks a bit dangerous, but I think it can be tamed"))
        end
    end,
...
mobs:spawn({
    name = "nh_mob:karibo",
    nodes = {"air"},
    neighbors = {"nh_nodes:ice", "nh_nodes:snow"},
    max_light = 15,
    interval = 120,
    chance = 300,
    active_object_count = 2,
...
mobs:register_mob("nh_mob:giantcrab", {
    ...
    drops = {
    	{name = "nh_nodes:redcrystal", chance = 1, min = 1, max = 3},  -- 1-3 cristais (sempre)
    },
    ...
    on_rightclick = function(self, clicker)
        if clicker:is_player() then
            ...
            if name == "" then
                ...
                core.chat_send_player(clicker:get_player_name(), S("Crrrr!"))  
            end
        end
    end,
