        ...
        if nearest_player then
            core.after(0.4, function()
                if self.object and self.object:is_valid() then
                    self.object:punch(nearest_player, 1.0, { full_punch_interval = 1.0, damage_groups = { fleshy = 1 }, }, nil)
                end
            end)
        end
    end,
...
mobs:register_mob("nh_mob:slime2", {
    ...
    after_activate = function(self, staticdata, def, dtime) self.object:set_properties({ use_texture_alpha = true, textures = { "planaria_slime2.png", "planaria_slime2.png", }, })
        -- Procura jogador próximo e já começa a atacar
        local pos = self.object:get_pos()
        local nearest_player = nil
        local nearest_dist = self.view_range or 7
        ...
        if nearest_player then core.after(0.4, function() if self.object and self.object:is_valid() then self.object:punch(nearest_player, 1.0, { full_punch_interval = 1.0, damage_groups = { fleshy = 1 }, }, nil) end end) end
    end,
    walk_velocity = 1,
    run_velocity = 2,
    view_range = 7,
...
mobs:register_mob("nh_mob:slime3", {
    ...
    after_activate = function(self, staticdata, def, dtime) self.object:set_properties({ use_texture_alpha = true, textures = { "planaria_slime2.png", "planaria_slime2.png", }, })
        ...
        if nearest_player then core.after(0.2, function() if self.object and self.object:is_valid() then self.object:punch(nearest_player, 1.0, { full_punch_interval = 1.0, damage_groups = { fleshy = 1 }, }, nil) end end) end
    end,
    walk_velocity = 1,
    run_velocity = 2,
    view_range = 7,
...
mobs:register_mob("nh_mob:visage2", {
    ...
    after_activate = function(self, staticdata, def, dtime)
        ...
        if nearest_player then             -- Pequeno delay para o mob terminar de spawnar antes do punch
            core.after(0.2, function() if self.object:is_valid() then self.object:punch(nearest_player, 1.0, { full_punch_interval = 1.0, damage_groups = { fleshy = 1 }, -- 1 de dano simbólico
                    }, nil)
                end
            end)
        end
...
mobs:register_mob("nh_mob:giantcrab", {
    ...
    on_rightclick = function(self, clicker)
        if clicker:is_player() then
            local item = clicker:get_wielded_item()
            local name = item:get_name()
            if name == "" then core.chat_send_player(clicker:get_player_name(), S("?!")) else core.chat_send_player(clicker:get_player_name(), S("Crrrr!")) end
        end
    end,
    sounds = { random = "vulto_som", damage = "vulto_hurt", },
    custom_attack = function(self, to_attack)
    ...
    after_activate = function(self, staticdata, def, dtime)
        ...
        core.after(0.2, function()
            ...
            if nearest_player then self.object:punch(nearest_player, 1.0, { full_punch_interval = 1.0, damage_groups = { fleshy = 1 }, }, nil) end
        end)
    end,
    do_custom = function(self, dtime)
