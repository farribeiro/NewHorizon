...
local c = core
c.log("action", "[MOVES] NH init.lua loaded")
...
local function is_walkable(pos)
    local n = c.get_node_or_nil(pos)
    if not n then return false end
    local reg = c.registered_nodes[n.name]
...
local function wall_at(check_x, base_y, check_z, height)
    for i = 0, height - 1 do
        if not is_walkable(xyz(check_x, base_y + i, check_z)) then
            return false
        end
    end
    return true
...
c.register_globalstep(function(dtime)
    ...
    for _, player in ipairs(c.get_connected_players()) do
        ...
        if controls.sneak and not crawling[name] then
            if not crouching[name] then
                crouching[name] = true
                crawling[name] = false
                player:set_physics_override({speed = 0.7, jump = 1})
                player:set_properties({
                    eye_height = 1.0,
    ...
    for _, player in ipairs(c.get_connected_players()) do
        ...
        local below = xyz(pos.x, pos.y - 0.15, pos.z)
        ...
        for _, yoff in ipairs(HEIGHT_OFFS) do
            if is_walkable(xyz(check_x, pos.y + yoff, check_z)) then base_wall_found = true break end
        end
        ...
        if is_climbing[name] and (not ctrl.jump or not base_wall_found) then is_climbing[name] = false end
        ...
        if jump_just_pressed and walljump_wall and walljump_ready and air_jumps[name] >= 1 and air_jumps[name] < 2 then
            ...
            player:add_velocity(xyz(-dir.x * 6, 4.5, -dir.z * 6))
        ...
        elseif ctrl.jump and not jump_just_pressed and not climb_blocked and
            ...
            local vel = player:get_velocity() or xyz(0)
            player:set_velocity(xyz(vel.x * HORIZ_FRICTION, CLIMB_SPEED, vel.z * HORIZ_FRICTION))
            moves_player_states[name] = "climb"
            player:set_physics_override({gravity = 0, jump = 0})
        -- PADRÃO — física normal
        else if not is_climbing[name] then player:set_physics_override({gravity = 1, jump = 1}) end
        end
