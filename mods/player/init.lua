...
local c = core
c.log("action", "[PLAYER] NH init.lua loaded")
...
local function hunger_timer()
    for _, player in ipairs(c.get_connected_players()) do
        ...
        if hunger_data[name] then
            ...
            if hunger_data[name].level == 0 then player:set_hp(player:get_hp() - 1) end
            -- Regenera vida se a fome estiver alta
            if hunger_data[name].level >= 18 and player:get_hp() < 20 then heal_player(player, 1) end
        end
    end
    c.after(30, hunger_timer) -- Chama a função novamente após 30 segundos
end
-- Inicia o timer de fome
c.after(30, hunger_timer)

c.register_on_joinplayer(function(player)
    local name = player:get_player_name()
    -- Remove o HUD de barras
    player:hud_set_flags({healthbar = false, breathbar = false}) -- Remove a barra de coração padrão / breath padrão
    ...
    c.after(0.1, function()
        ...
        if saved_hp > 0 then
            ...
            if health_hud[name] then player:hud_change(health_hud[name], "number", saved_hp) end
        end
    end)
    -- INICIALIZA O ESTADO DA HOTBAR
    if not hotbar_state then hotbar_state = {} end
    hotbar_state[name] = {current_size = 2, needs_update = false}
    ...
    c.after(0.2, function()
        if not (player and player:is_player()) then return end
        player:set_physics_override({stepheight = 1.1, jump = 1})
    end)
    ...
    c.after(0.3, function()
        ...
        hunger_data[name] = {level = saved_hunger, hud_id = nil}
...
c.register_on_leaveplayer(function(player)
    ...
    local meta = player:get_meta()
    -- Salva a fome
    if hunger_data[name] then meta:set_int("hunger_level", hunger_data[name].level) end
    -- Salva a vida
...
function get_hunger(player)
    local name = player:get_player_name()
    if hunger_data[name] then return hunger_data[name].level end
    return 0
end
...
function add_item_to_visible_slots(player, itemstack)
    ...
    c.add_item(player:get_pos(), itemstack)
    return false
end
...
c.register_globalstep(function(dtime)
    for _, player in ipairs(c.get_connected_players()) do
        ...
        if breath_hud[name] then
            local breath = player:get_breath()
            local head_pos = xyz(player:get_pos().x, player:get_pos().y + player:get_properties().eye_height, player:get_pos().z)
            local head_node = c.get_node(head_pos).name
            -- Mostra/esconde a barra baseado se está submerso
            if c.get_item_group(head_node, "water") > 0 then player:hud_change(breath_hud[name], "number", breath * 2)
            else player:hud_change(breath_hud[name], "number", 0) -- Esconde a barra quando não está na água
            end
        end
        ...
        if is_moving and not auto_jump_cooldown[player_name] then
            ...
            if on_ground then
                ...
                if length > 0 then
                    ...
                    local node_feet = c.get_node(check_pos_feet)
                    local node_waist = c.get_node(check_pos_waist)
                    local node_head = c.get_node(check_pos_head)
                    -- Define quais blocos são "sólidos" (obstáculos)
                    local function is_solid(node_name)
                        local node_def = c.registered_nodes[node_name]
                    ...
                    if is_solid(node_waist.name) and 
                       is_passable(node_head.name) then
                        -- Faz o jogador pular
                        player:add_velocity({x = 0, y = 7.0, z = 0}) -- Força do pulo
